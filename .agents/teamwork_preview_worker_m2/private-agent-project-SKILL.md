---
name: private-agent-project
description: >
  Project-specific skill for the PrivateAgent Android automation agent.
  Loaded automatically when working inside the private-agent workspace.
  Contains architecture specifications, coding conventions, known issues, and roadmap guidelines.
---
...
```
User Input → AiService (LLM) → parseAction(JSON) → ActionHandler (Dispatcher)
    ↓                                                      ↓
  Chat UI                                          Specialized Services
                                                         ↓
                                                  TaskExecutor (Multi-Step)
                                                         ↓
                                              Screen Dump → LLM → Action → Repeat
```
...
4. **Token Bloat:** TaskExecutor adds duplicate system instructions to the chat history.
5. **Sync Issues:** `availableActions` array does not match actual supported actions in ActionHandler.
